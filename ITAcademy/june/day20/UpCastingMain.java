...
public class UpCastingMain {
	...
	public static void main(String[] args) {
		...
		System.out.println("UpCasting의 객체");
		UpCastingParent upc = new UpCastingChild();
		//업캐스팅 객체를 생성, 타입은 부모클래스만 UpCastingParent 이고,
		//생성자 호출은 자식클래스인 UpCastingChild의 것을 호출한다.
		System.out.println("upc.add()= " + uc.add()+ "\n");
		//add를 호출하면 재정의된 메서드가 호출되어 x+y+z의 결과를 반환.
		//system.out.println(upc.sub()= " + up.sub());
		//sub()를 호출하는데, upc의 타입이 UpCasintgParent 이므로
		//이 클래스에는 sub() 이 정의되지 않음. 그래서 에러가 발생.
		//이처럼 타입은 부모, 실행되는 형태는 자식인 것을 업캐스팅이라 한다.
	}
